...
-- operator. Replace it with a partial unique that keeps only the operator-wide
-- link (boat_id null) singular; the existing capture_links_boat_uniq already
-- keeps at most one link per boat. No token is read or written, so every QR
-- code already printed keeps working exactly as before.
