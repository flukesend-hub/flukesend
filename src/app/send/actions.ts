...
export async function createSend(
  ...
  const { data: branding } = await supabase
    .from("branding")
    .select(
      "retention_days, brand_color, accent_color, header_text_color, font_key, copy_overrides, logo_url, default_message, reply_to_email, website_url, facebook_url, instagram_url, tiktok_url, youtube_url, x_url",
    )
    .eq("operator_id", operatorId)
    .maybeSingle();
