...
export type CreateSendResult =
  | { error: string }
  | { ok: true; deliveryId: string; recipientCount: number };
...
export async function createSend(
  ...
  // email here. Each row is its own guest with its own gallery link.
  const recipientRows = emails.map((email) => ({
    delivery_id: delivery.id,
    email,
  }));
  const { error: rErr } = await supabase.from("recipients").insert(recipientRows);
  if (rErr) {
    await cleanupFailedSend(delivery.id, input.photos.map((p) => p.storageKey));
    return { error: "Could not save the recipients. Try again." };
  }

  return { ok: true, deliveryId: delivery.id, recipientCount: emails.length };
}
